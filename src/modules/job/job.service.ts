...
export class JobService {
  ...
  async cancelJob(uuid: string): Promise<Job> {
    const selectedJob = await this.jobRepository.findOne(uuid);

    const newShifts = selectedJob.shifts && selectedJob.shifts.map(shift => ({ 
      ...shift, 
      isCancelled: true,
    }));
